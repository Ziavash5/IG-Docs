...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Stage + pillar landing pages (corridor-prefixed).
  for (const s of journey) {
    urls.push({ url: `${BASE}/${c}/${s.slug}`, changeFrequency: "monthly", priority: 0.5 });
    for (const p of s.pillars) urls.push({ url: `${BASE}/${c}/${s.slug}/${p.slug}`, changeFrequency: "monthly", priority: 0.6 });
  }
